...
- `catalog.youku`
- `search.pansou`
- `automation.dingtalk_robot`
- `automation.email`
- `automation.feishu_bot`
- `automation.webhook`
- `automation.wecom_app`
- `assistant.wecom_bot`
...
- 任何插件功能、行为、按钮、任务动作、配置或对外文案的调整，都必须同步升级该插件版本号。
- `plugin.json` 里的 `version` 要和后端 `plugin_version` 保持一致。
- 提交前执行 `python tools/validate_plugin_versions.py` 做一次版本一致性校验。
- 调整完成后请提交并推送到远程仓库，避免远端仍停留在旧版本。
